...
instance CodeGenerator Stmt where
  ...
  codeGeneration state (While e s) =
    let (c,state') = codeGeneration state e
    in let (c1,state'') = codeGeneration state' s
       in ([OnlyLabel (genLabel state'' 0)] ++ c
           ...
           ++ c1
           ++ [COp $ Op1 "jmp" (Label (genLabel state'' 0)),
               OnlyLabel (genLabel state'' 1)],
           addLabelCount 2 state'')

instance CodeGenerator Expr where
  codeGeneration state (Syntax.AST.Const i) =
    ([COp $ Op2 "mov" (Reg Eax) (CodeGen.AsmCode.Const i)],state)
  codeGeneration state (Ident (Identifier s)) =
    ([COp $ Op2 "mov" (Reg Eax) (GlobalRef s)],state)
  codeGeneration state (Ident (STableKey i)) =
    let (SVar sym) = fromJust $ M.lookup i (table state)
    in ([COp $ Op2 "mov" (Reg Eax) (Ref Ebp (vadr sym))],state)
  codeGeneration state (Assign (Identifier s) expr) =
    let (rcode,state') = codeGeneration state expr
    in (rcode ++ [COp $ Op2 "mov" (GlobalRef s) (Reg Eax)],state')
  codeGeneration state (Assign (STableKey i) expr) =
    let (SVar sym) = fromJust $ M.lookup i (table state)
    in let (rcode,state') = codeGeneration state expr
       in (rcode ++ [COp $ Op2 "mov" (Ref Ebp (vadr sym)) (Reg Eax)],state')
  codeGeneration state (UMinus expr) =
    let (codes,state') = codeGeneration state expr
    in (codes++[COp $ Op1 "neg" (Reg Eax)],state')

  codeGeneration state (L_AND e1 e2) =
    let state' = allocateLoc SInt state
       ...
       in let (c2,state''') = codeGeneration state'' e2
          in ([COp $ Op2 "mov dword" (Ref Ebp $ top state) (CodeGen.AsmCode.Const 0)]
              ...
              ++ [COp $ Op2 "cmp" (Reg Eax) (CodeGen.AsmCode.Const 0)
                 ...
                 ,OnlyLabel (genLabel state 0)
                 ,COp $ Op2 "mov" (Reg Eax) (Ref Ebp $ top state)],
              state''')

  codeGeneration state (L_OR e1 e2) =
    let state' = allocateLoc SInt state
    in let (c1,state'') = codeGeneration (addLabelCount 1 state') e1
       in let (c2,state''') = codeGeneration state'' e2
  ...
  codeGeneration state (FunCall (Identifier s) elist) =
    let (codes,state') = f (reverse elist) state
